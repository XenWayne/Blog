...
- 一旦你决定向我发起交接，我将会提供一个MUA_Union_Key和一个文件包，其中包含了本文档中提到的所有服务端、存档、配置文件、数据库文件、站点文件等。发起交接流程的同天我将在发送相关文件后{% label "关停LightZone服务" red %}以免与你的服务部署冲突。
...
- 联系我拿到文件包后，你可以不按本文介绍的现有技术架构组织服务，但请确保数据平稳迁移，并向辽宁大学Minecraft交流协会说明、在LightZone文档站更新相关链接并保留之前相关贡献人员的署名。
...
{% note warning %}
Velocity的modern转发模式需要自设一个密钥，为了安全起见，你拿到的文件包中代理端和子服的配置文件中均不会包含我设定的密钥，除此之外，部分插件的MySQL数据库连接信息、Velocity二层代理的MUA文件等也不会包含在文件包中，这些信息需要你自行配置。
{% endnote %}
